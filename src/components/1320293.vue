<template>
    <div>
        <v-subheader>办公用品入库</v-subheader>
        <v-data-table
                height="300"
                :items="workArticlePutDetails"
                :headers="headers"
...
<script>
    import {getById} from "../api/workMaterialPut";
    ...
    export default {
        ...
        data:() => ({
            workArticlePut:{},
            workArticlePutDetails:[],
            headers:[
                {text: "办公用品名称", value: 'material.name', width: '20%'},
                {text: "入库数", value: 'sum', width: '20%'},
                {text: "单价", value: 'price', width: '10%'},
        ...
        methods:{
            loadData(){
                if (this.frameId) {
                    getById({id:this.frameId}).then(res => {
                        console.log(res)
                        this.workArticlePut = res.workArticlePut
                        this.workArticlePutDetails = res.worArticlePutDetails
                    })
                }
            },
            reset(){
