      ...
      <v-container id="printc">
        <v-row class="border" justify="center">
          ...
          <v-col cols="12" style="min-height: 200px" class="border-b">
            <table width="100%">
              <caption class=" text-center">分包合同盖章申请</caption>
              <tr>
                <td class="border text-center" width="30%">合同名称</td>
                <td class="border text-center" width="70%">{{ proSubcontract.name }}</td>
              </tr>
              <tr>
                <td class="border text-center" width="30%">乙方</td>
                <td class="border text-center" width="70%">{{ proSubcontract.partyB.name }}</td>
              </tr>
              <tr>
                <td class="border text-center" width="30%">合同金额</td>
                <td class="border text-center" width="70%">{{ proSubcontract.money }}元</td>
              </tr>
              <tr>
                <td class="border text-center" width="30%">签订日期</td>
                <td class="border text-center" width="70%">{{ proSubcontract.signDate }}</td>
              </tr>
              <tr>
                <td class="border text-center" width="30%">备注</td>
                <td class="border text-center" width="70%">{{ proSubcontract.remark }}</td>
              </tr>
            </table>
          </v-col>
...
export default {
  ...
  methods: {
    load() {
      proSubcontract.getSubcontractById({id: this.message.frameId}).then(res => {
        if (res.id == null) {
