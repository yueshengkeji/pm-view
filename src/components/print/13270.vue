        ...
        <v-row class="border" justify="center">
          ...
          <v-col cols="12" style="min-height: 200px" class="border-b">
            ...
            <table style="width:100%">
              <thead>
              <tr>
                <td class="border-black" width="20%">序号</td>
                <td class="border-black" width="10%">系统名称</td>
                <td class="border-black" width="10%">名称</td>
                <td class="border-black" width="15%">型号</td>
                <td class="border-black" width="15%">品牌</td>
                <td class="border-black" width="10%">单位</td>
                <td class="border-black" width="20%">计划数量</td>
                <td class="border-black" width="20%">单价</td>
                <td class="border-black" width="20%">合计</td>
                <td class="border-black" width="20%">技术参数</td>
              </tr>
              </thead>
              <tbody>
              <tr v-for="(item,index) in data.materialList" :key="item.id">
                <td class="border-black">{{ index+1 }}</td>
                <td class="border-black">{{ item.taskId }}</td>
                <td class="border-black">{{ item.material.name }}</td>
                <td class="border-black">{{ item.material.model }}</td>
                <td class="border-black">{{ item.material.brand }}</td>
                <td class="border-black">{{ item.material.unit.name }}</td>
                <td class="border-black">{{ item.planSum }}</td>
                <td class="border-black">{{ item.planPrice }}</td>
                <td class="border-black">{{ item.money }}</td>
                <td class="border-black">{{ item.cnfStr }}</td>
              </tr>
              </tbody>
            </table>
          </v-col>
...
export default {
  ...
  mounted() {
    this.$nextTick(function () {
      plan.getPlanById(this.message.frameId).then(result => {
        this.project = `项目名称：${result.project.name}`;
        this.data = result;
        this.print()
      })
    })
