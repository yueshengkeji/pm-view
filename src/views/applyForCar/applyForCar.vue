  ...
  <v-card>
    <v-card-title>
      <v-row>
        ...
        >
          ...
          <v-btn small :loading="loading2" @click="exportExcel" class="ml-1">
            导出
          </v-btn>
        </v-col>
        <v-col
    ...
    >
      <template v-slot:item.action="{item}">
        <v-btn outlined x-small color="error" @click="delHandler(item)">删除</v-btn>
      </template>
    </v-data-table>
  </v-card>
</template>
  ...
  methods: {
    delHandler(item) {
      if (item.checkState == '未审核') {
        this.confirm(`确定删除第${item.index}行，${item.remark}用车申请？`).then(() => {
          applyForCarApi.delApply(item.id).finally(this.loadApplyForCar)
        })
      }else{
