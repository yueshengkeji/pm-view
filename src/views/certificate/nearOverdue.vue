<template>
    <div>
            ...
            <v-col>
                <v-data-table :items="items" :headers="headers" :search="search">
                    <template v-slot:top>
                        <v-row>
                            <v-col lg="3">
                                <v-btn class="mr-3" icon @click="$router.go(-1)"><v-icon>mdi-arrow-left</v-icon>返回</v-btn>
                                <span class="blue--text" @click="upStaff">过期提醒人员：{{staff.name}}</span>
                                <v-icon color="blue" small @click="upStaff">mdi-pencil</v-icon>
                            </v-col>
                            <v-col lg="2" offset-lg="7">
        ...
        <v-dialog v-model="staffDialog" width="50%">
            <v-card class="pa-5">
                <v-autocomplete item-value="id" :items="user" item-text="name" return-object :search-input.sync="searchUser" label="更改消息推送人员" v-model="sinkUser"></v-autocomplete>
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn @click="staffSink" color="primary">确定</v-btn>
                    <v-btn @click="staffDialog = false">取消</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </div>
...
<script>
    import {deleteCer, nearOverdue, querySinkMsg, sinkMsg, updateCer} from "@/api/certificate";
