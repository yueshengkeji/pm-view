                ...
                <div class="box">
                    <div class="box-youhuiquan">
                        ...
                        <div class="box-desc">
                            ...
                            <div class="desc-leave">{{experienceCouponItem.newUserFlag == 0 ? ("剩余" + (experienceCouponItem.totalNum - experienceCouponItem.sentNum) + "张") : ""}}</div>
                            <div class="desc-use-time">{{(experienceCouponItem.startTime != null && experienceCouponItem.endTime != null) ? ("使用时间:" + experienceCouponItem.startTime + "-" + experienceCouponItem.endTime) : ""}}</div>
                        </div>
                    </div>
                </div>
            </v-row>
...
import {insert} from "@/api/jxhExperienceCoupon";
import FileUpload from "@/components/fileUpload.vue";
import {getConfig} from "@/api/systemConfig";

export default {
    name: 'addExperienceCoupon',
    ...
    methods: {
        ...
        submitAdd() {
            ...
            return insert(this.experienceCouponItem).then(res => {
                return res
            })
        },
        reset(){
        ...
        fileChange(res){
            if (res.files.length > 0) {
                console.log('path',this.serverHost + this.ftpFolder + "/" + res.files[0].ftpPath)
