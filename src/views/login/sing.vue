  ...
  <v-container :class="loginClass" fluid fill-height>
    <v-row justify="center" v-if="dingLoginFlag == false">
      <v-col v-if="!autoLoading" lg="4" md="4" sm="5" cols="12">
        <v-card>
          <h1 class="text-center mb-5 pt-3">系统登录</h1>
      ...
      <v-col class="text-center" v-if="autoLoading">
        ...
        <p v-if="showRefreshMsg">如果加载时间超过5秒，表示系统有新版本升级，请<a href="###"
                                                                               @click="window.location.reload()">立即刷新</a>即可进入新版本系统
        </p>
...
export default {
  ...
  created() {
    ...
    if (dd.env.platform !== "notInDingTalk") {
      ...
      if (ua.match(/MicroMessenger/i) == 'micromessenger' && this.autoWxLogin) {
        let redirect = redirectPath || this.$route.query.redirect
        this.loginClass = this.loginClass.replace("login", "")
        this.autoLoading = true
        let code = this.$route.query.code
        console.log("code1",code)
        code = code == null ? this.$route.params.code : code
        console.log("code2",code,localStorage.getItem("oauth2"))
        if (code == null && localStorage.getItem("oauth2") != "1") {
          ...
          //获取微信code
          this.loadingMsg = "获取企业信息"
          wxConfig().then(result => {
            if (result) {
              let url = encodeURIComponent(result.NOTIFYIP + this.$router.options.base + "login")
          ...
          let oauth2Redirect = state || localStorage.getItem("oauth2Redirect")
  ...
  methods: {
    ...
    // },
    setDefaultUrl() {
      this.user.url = "http://" + window.location.host + this.$router.options.base + "login"
    },
    enter(event) {
      if (event.keyCode === 13) {
        this.login();
    ...
    login() {
      ...
      if (result) {
        this.remember()
        this.user.password = encrypt(this.inputPassword)
        this.autoLogin()
    ...
    loginSuccess(r) {
      window.setTimeout((rect) => {
        ...
        this.$router.replace({path: defaultUrl}).then(() => {
          window.clearTimeout(localStorage.getItem("timeout"))
        }).catch(e => {
