  ...
  <div>
    ...
    <v-data-table :items="contractReceiptList"
      ...
      <template v-slot:item.stateStr="{item}">
        <v-chip outlined small v-if="item.state == 0" color="red" link @click="showSate(item)">{{ item.stateStr }}
        </v-chip>
        <v-chip outlined small v-if="item.state == 1" color="green" link @click="showSate(item)">{{ item.stateStr }}
        </v-chip>
        <v-chip outlined small v-if="item.state == 9" color="grey" link @click="showSate(item)">{{ item.stateStr }}
        </v-chip>
      </template>
