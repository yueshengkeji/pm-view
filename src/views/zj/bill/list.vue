        ...
        <v-row dense>
            ...
            <v-col cols="2">
                <v-menu v-model="menu" transition="scale-transition" offset-y :close-on-content-click="false"
                    ...
                    <v-date-picker range v-model="date" no-title scrollable @change="dateChangeHandler"></v-date-picker>
                </v-menu>
            </v-col>
            <v-col cols="2">
...
<script>
import {exportBill, getBillList, listTermByConcat} from '@/api/zujin'
...
export default {
    ...
    watch: {
        ...
        listType: {
            handler() {
                if (this.listType == 'listToPay') {
                    this.stateItems = [
                        {value: 'wait', text: '待支付'},
                        {value: 'pay', text: '部分支付'},
                    ]
                    this.query.state = 'wait'
                } else if (this.listType == 'listPaid') {
                    this.stateItems = [
                        {value: '1', text: '已支付'},
                    ]
                    this.query.state = '1'
