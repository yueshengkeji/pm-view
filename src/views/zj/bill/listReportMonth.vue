    ...
    <div>
        <v-row no-gutters>
            <v-col cols="6"
                ...
                ></v-combobox>
                <v-card>
                    <v-card-title>{{searchYear}}年所出账单</v-card-title>
...
<script>
import {zjMonthReport} from "@/api/zjReport";
...
export default {
    ...
    methods: {
        ...
        load() {
            zjMonthReport({year: this.searchYear + ''}).then(res => {
        ...
        loadBills(event, row) {
            this.queryBillParams.startDate = row.item.date + '-01'
            this.queryBillParams.endDate = this.getLastDate(row.item.date.split('-')[0], row.item.date.split('-')[1])
            this.loadBillsData()
        },
        loadBillsData(){
