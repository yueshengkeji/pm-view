<template>
    <v-card class="pa-3">
        <v-form ref="zujinForm" class="pt-5">
            ...
            <v-row>
                ...
                <v-col sm="2">
                    <v-menu v-model="menu4" transition="scale-transition" offset-y :close-on-content-click="false"
                        ...
                        <v-date-picker v-model="data.endDatetime" no-title scrollable
                                       @change="$refs.menu4.save()"></v-date-picker>
                    </v-menu>
                </v-col>
                ...
                <v-col sm="2">
                    <v-menu v-model="menu7" transition="scale-transition" offset-y :close-on-content-click="false"
                        ...
                        <v-date-picker v-model="data.planDate" no-title scrollable
                                       @change="$refs.menu7.save()"></v-date-picker>
                    </v-menu>
                </v-col>
                ...
                <v-col sm="2">
                    <v-menu v-model="menu8" transition="scale-transition" offset-y :close-on-content-click="false"
                        ...
                        <v-date-picker v-model="data.openDate" no-title scrollable
                                       @change="$refs.menu8.save()"></v-date-picker>
                    </v-menu>
                </v-col>
            ...
            <v-row>
                ...
                <v-col sm="2">
                    <v-menu v-model="menu9" transition="scale-transition" offset-y :close-on-content-click="false"
                        ...
                        <v-date-picker v-model="data.signDate" no-title scrollable
                                       @change="$refs.menu9.save()"></v-date-picker>
                    </v-menu>
                </v-col>
                <v-col sm="3">
            ...
            <v-row>
                <v-col sm="2" style="margin-left: 50px">
                    <v-card>
                        <v-img
                                :src="data.businessLicenseImageUrl"
                                class="white--text align-end"
                                gradient="to bottom, rgba(0,0,0,.1), rgba(0,0,0,.5)"
                                height="200px"
                                @click="showImageItem(data.businessLicenseImageUrl)"
                        >
                            <v-card-title v-text="'请上传营业执照'"
                                          v-if="data.tenantType == 1 && data.businessLicenseImageUrl == 'null'"
                                          style="color: red"></v-card-title>
                            <v-card-title v-text="'营业执照'" v-else></v-card-title>
                        </v-img>

                        <v-card-actions>
                            <v-spacer></v-spacer>
                            <v-btn icon @click="showImageItem(data.businessLicenseImageUrl)">
                                <v-icon>mdi-eye-outline</v-icon>
                            </v-btn>
                            <file-upload :if-hide="true" @change="businessLicenseChange" :clear="businessLicenseClear"
                ...
                <v-col sm="2">
                    <v-card>
                        <v-img
                                ...
                                gradient="to bottom, rgba(0,0,0,.1), rgba(0,0,0,.5)"
                                height="200px"
                                @click="showImageItem(data.idImageUrl)"
                        >
                            <v-card-title v-text="'请上传法人/乙方身份证'"
                                          v-if="data.tenantType == 1 && data.idImageUrl == 'null'"
                                          style="color: red"></v-card-title>
                            <v-card-title v-text="'请上传法人/乙方身份证'"
                                          v-else-if="data.tenantType == 2 && data.idImageUrl == 'null'"
                                          style="color: red"></v-card-title>
                            <v-card-title v-text="'法人/乙方身份证'" v-else></v-card-title>
                        </v-img>

                        <v-card-actions>
                            <v-spacer></v-spacer>
                            <v-btn icon @click="showImageItem(data.idImageUrl)">
                                <v-icon>mdi-eye-outline</v-icon>
                            </v-btn>
                            <file-upload :if-hide="true" @change="idChange" :clear="businessLicenseClear"
                                         :accept-type="'image/*'"></file-upload>
                        </v-card-actions>
                    </v-card>
                </v-col>
                <v-col sm="2">
                    <v-card>
                        <v-img
                                :src="data.tradeMarkImageUrl"
                                class="white--text align-end"
                                gradient="to bottom, rgba(0,0,0,.1), rgba(0,0,0,.5)"
                                height="200px"
                                @click="showImageItem(data.tradeMarkImageUrl)"
                        >
                            <v-card-title v-text="'请上传商标注册证'"
                                          v-if="data.tenantType == 1 && data.tradeMarkImageUrl == 'null'"
                                          style="color: red"></v-card-title>
                            <v-card-title v-text="'商标注册证'" v-else></v-card-title>
                        </v-img>

                        <v-card-actions>
                            <v-spacer></v-spacer>
                            <v-btn icon @click="showImageItem(data.tradeMarkImageUrl)">
                                <v-icon>mdi-eye-outline</v-icon>
                            </v-btn>
                            <file-upload :if-hide="true" @change="tradeMarkChange" :clear="businessLicenseClear"
                                         :accept-type="'image/*'"></file-upload>
                        </v-card-actions>
                    </v-card>
                </v-col>
                <v-col sm="2">
                    <v-card>
                        <v-img
                                :src="data.tradeUseImageUrl"
                                class="white--text align-end"
                                gradient="to bottom, rgba(0,0,0,.1), rgba(0,0,0,.5)"
                                height="200px"
                                @click="showImageItem(data.tradeUseImageUrl)"
                        >
                            <v-card-title v-text="'请上传商标使用授权书'"
                                          v-if="data.tenantType == 1 && data.tradeUseImageUrl == 'null'"
                                          style="color: red"></v-card-title>
                            <v-card-title v-text="'商标使用授权书'" v-else></v-card-title>
                        </v-img>

                        <v-card-actions>
                            <v-spacer></v-spacer>
                            <v-btn icon @click="showImageItem(data.tradeUseImageUrl)">
                                <v-icon>mdi-eye-outline</v-icon>
                            </v-btn>
                            <file-upload :if-hide="true" @change="tradeUseChange" :clear="businessLicenseClear"
                                         :accept-type="'image/*'"></file-upload>
                        </v-card-actions>
                    </v-card>
                </v-col>
                <v-col sm="2">
                    <v-card>
                        <v-img
                                :src="data.brandAuthorizationImageUrl"
                                class="white--text align-end"
                                gradient="to bottom, rgba(0,0,0,.1), rgba(0,0,0,.5)"
                                height="200px"
                                @click="showImageItem(data.brandAuthorizationImageUrl)"
                        >
                            <v-card-title v-text="'请上传品牌授权书'"
                                          v-if="data.tenantType == 1 && data.brandAuthorizationImageUrl == 'null'"
                                          style="color: red"></v-card-title>
                            <v-card-title v-text="'请上传品牌授权书'"
                                          v-else-if="data.tenantType == 2 && data.brandAuthorizationImageUrl == 'null'"
                                          style="color: red"></v-card-title>
                            <v-card-title v-text="'品牌授权书'" v-else></v-card-title>
                        </v-img>

                        <v-card-actions>
                            <v-spacer></v-spacer>

                            <v-btn icon @click="showImageItem(data.brandAuthorizationImageUrl)">
                                <v-icon>mdi-eye-outline</v-icon>
                            </v-btn>

                            <file-upload :if-hide="true" @change="brandAuthorizationChange"
                                         :clear="businessLicenseClear"
                                         :accept-type="'image/*'"></file-upload>
                    ...
                    <v-tabs-items v-model="tab">
                        <v-tab-item key="zl">
                            <v-data-table :items.sync="zlItems" :headers="headers" hide-default-footer
                                ...
                                <template v-slot:item.action="{item}">
                                    ...
                                    <v-btn color="error" x-small outlined @click="deleteMoneyHandler(item)"
                                           :disabled="updateFlag">删除
                                    </v-btn>
                                </template>
                            </v-data-table>
                        </v-tab-item>
                        <v-tab-item key="bzj">
                            <v-data-table :items.sync="bzjItems" :headers="bzjHeader" hide-default-footer
                                          :items-per-page="-1">
                                <template v-slot:item.action="{item}">
                                    <v-btn color="error" x-small outlined @click="deleteBzjHandler(item)"
                                           :disabled="updateFlag">删除
                                    </v-btn>
                                </template>
                            </v-data-table>
                        ...
                        <v-tab-item key="tgf">
                            <v-data-table :items.sync="tgfItems" :headers="tgfHeader" hide-default-footer
                                          :items-per-page="-1">
                                <template v-slot:item.action="{item}">
                                    <v-btn color="error" x-small outlined @click="deleteTgfHandler(item)"
                                           :disabled="updateFlag">删除
                                    </v-btn>
                                </template>
                            </v-data-table>
        ...
        <v-card-actions>
            ...
            <v-col sm="2">
                <file-upload ref="file" @change="fileChangeHandler" :clear="clear"></file-upload>
            </v-col>
            <v-radio-group
                    v-model="data.type"
                    mandatory
            ...
            <v-btn :disabled="data.type == 9 || offGen" @click="genNextHandler" :loading="genLoading">生成下一期账单
            </v-btn>
            <v-btn :disabled="data.type == 9" @click="printContractWord">打印</v-btn>
            <v-btn v-if="!updateFlag" :disabled="data.type == 9" @click="insertZujin" color="primary"
                   :loading="loading">
                确定
            </v-btn>
            <v-btn v-if="!updateFlag" :disabled="data.type == 9" @click="saveData" :loading="loading">仅保存数据</v-btn>
                    ...
                    <v-row>
                        ...
                        <v-col cols="6">
                            <v-menu v-model="menu" transition="scale-transition" offset-y
                                ...
                                <v-date-picker v-model="t.startDate" no-title scrollable
                        ...
                        <v-col cols="6">
                            <v-menu v-model="menu2" transition="scale-transition" offset-y
                                ...
                                <v-date-picker v-model="t.endDate" no-title scrollable
                                               @change="$refs.menu2.save()"></v-date-picker>
                            </v-menu>
                        </v-col>
                        <v-col cols="3">
                            <v-select v-model="t.type" :items="typeItems" label="计费方式*"
                                      @change="typeChangeHandler"></v-select>
                        </v-col>
                        <v-col cols="3">
                        ...
                        <v-col cols="3">
                            <v-select v-model="t.unit" :items="unitItems" label="计费单位*"
                                      :rules="rules.unit"></v-select>
                        </v-col>
                        <v-col cols="3">
                        ...
                        <v-col cols="4">
                            <v-radio-group style="margin-top: -30px" v-model="t.billType" label="账单类型"
                                ...
                                <v-radio :value="0" label="下期"
                                         title="下期=到达每月账单日，生成下一个对应的‘月/季度’账单"></v-radio>
                                <v-radio :value="1" label="本期"
                                         title="本期=到达每月账单日，生成本月应收账单；"></v-radio>
        ...
        <v-dialog v-model="bzjDialog" @click="bzjDialog = false" width="30%">
            <v-card class="pa-5">
                ...
                <v-form ref="bzjForm">
                    ...
                    <v-text-field v-if="bzj.type == '履约保证金' && bzj.payAll == false"
                                  :rules="bzjRules.requiredProperty"
                    ...
                    <v-menu v-model="bzjMenu2" transition="scale-transition" offset-y>
                        <template v-slot:activator="{ on, attrs }">
                            <v-text-field v-model="bzj.datetime" :rules="bzjRules.datetime" label="区间截止日期*"
                                          readonly
                                          v-bind="attrs"
                                          v-on="on"></v-text-field>
                        </template>
                    ...
                    <v-menu v-model="bzjMenu3" transition="scale-transition" offset-y>
                        <template v-slot:activator="{ on, attrs }">
                            <v-text-field v-model="bzj.endDate" :rules="bzjRules.datetime" label="缴费截止日期*"
                                          readonly
                                          v-bind="attrs"
                                          v-on="on"></v-text-field>
                        </template>
        ...
        <v-dialog v-model="tgfDialog" width="50%">
            <v-card class="pa-3">
                <v-form ref="tgfForm">
                    <v-row>
                        <v-col cols="12">
                            <v-text-field v-model="tgfItem.name" label="推广费名称*"
                                          :rules="[v => !!v || '请输入推广名称']"
                            ></v-text-field>
                        </v-col>
                        <v-col cols="3">
                        ...
                        <v-col cols="4">
                            <v-menu v-model="bzjMenu4" transition="scale-transition" offset-y>
                                <template v-slot:activator="{ on, attrs }">
                                    <v-text-field v-model="tgfItem.payEndDate" :rules="bzjRules.datetime"
                                                  label="缴费截止日期*" readonly
                                                  v-bind="attrs"
                                                  v-on="on"></v-text-field>
                                </template>
                        ...
                        <v-col cols="4">
                            <v-menu v-model="bzjMenu5" transition="scale-transition" offset-y>
                                <template v-slot:activator="{ on, attrs }">
                                    <v-text-field v-model="tgfItem.startDate" :rules="bzjRules.datetime"
                                                  label="推广开始日期*" readonly
                                                  v-bind="attrs"
                                                  v-on="on"></v-text-field>
                                </template>
                                <v-date-picker @change="bzjMenu5 = false" v-model="tgfItem.startDate" no-title
                                               scrollable></v-date-picker>
                            </v-menu>
                        </v-col>
                        <v-col cols="4">
                            <v-menu v-model="bzjMenu6" transition="scale-transition" offset-y>
                                <template v-slot:activator="{ on, attrs }">
                                    <v-text-field v-model="tgfItem.endDate" :rules="bzjRules.datetime"
                                                  label="推广截止日期*" readonly
                                                  v-bind="attrs"
                                                  v-on="on"></v-text-field>
                                </template>
                                <v-date-picker @change="bzjMenu6 = false" v-model="tgfItem.endDate" no-title
                                               scrollable></v-date-picker>
                            </v-menu>
                        </v-col>
                    </v-row>
        ...
        <v-dialog v-model="showFilesDialog" width="60%">
            <contract-files :type="1" :data="data"></contract-files>
        </v-dialog>
        <v-dialog v-model="showImageDialog" width="40%">
            <v-img :src="showImageUrl"></v-img>
        </v-dialog>
    </v-card>
</template>
...
export default {
    ...
    data: () => ({
        ...
        tgfHeader: [
            ...
            {value: 'startDate', text: '推广开始日期'},
            {value: 'endDate', text: '推广截止日期'},
            {value: 'action', text: '操作'}
        ],
        tgfItem: {
            id: null,
    ...
    created() {
        ...
        getConfig("JX_AUTH").then(result => {
            if (result.id != null) {
                let roles = result.value.split(";")
                roles.forEach(val => {
                    console.log("JX_AUTH", val, this.$store.state.user.roles)
                    if (this.$store.state.user.roles.indexOf(val) != -1) {
                        this.editAuth = true
                    }
                })
            }
    ...
    methods: {
        ...
        loadById(id) {
            if (id) {
                queryById(id).then(result => {
                    ...
                    if (!result.businessLicenseImageUrl) {
                        result.businessLicenseImageUrl = 'null'
                    }
                    if (!result.idImageUrl) {
                        result.idImageUrl = 'null'
                    }
                    if (!result.tradeMarkImageUrl) {
                        result.tradeMarkImageUrl = 'null'
                    }
                    if (!result.tradeUseImageUrl) {
                        result.tradeUseImageUrl = 'null'
                    }
                    if (!result.brandAuthorizationImageUrl) {
                        result.brandAuthorizationImageUrl = 'null'
                    }
                    this.data = result
                    ...
                    if (!this.editAuth) {
                        getMessageByFrameId(id + "-zujin").then(msg => {
                            if (msg && msg.state == 2) {
        ...
        saveData() {
            ...
            if (this.data.tenantType == 1 && this.data.businessLicenseImageUrl != 'null' && this.data.idImageUrl != 'null'
                && this.data.tradeMarkImageUrl != 'null' && this.data.tradeUseImageUrl != 'null' && this.data.brandAuthorizationImageUrl != 'null') {
                tenantValid = true
            }
            if (this.data.tenantType == 2 && this.data.idImageUrl != 'null' && this.data.brandAuthorizationImageUrl != 'null') {
                tenantValid = true
            }
            let valid = this.$refs['zujinForm'].validate();
            if (valid && tenantValid) {
        ...
        businessLicenseChange(res) {
            if (res.files.length > 0) {
                this.data.businessLicenseImageUrl = this.serverHost + res.files[0].ftpPath
                this.businessLicenseClear = true
            } else if (res.files.length == 0) {
                this.businessLicenseClear = false
            }
        },

        idChange(res) {
            if (res.files.length > 0) {
                this.data.idImageUrl = this.serverHost + res.files[0].ftpPath
                this.businessLicenseClear = true
            } else if (res.files.length == 0) {
                this.businessLicenseClear = false
            }
        ...
        tradeMarkChange(res) {
            if (res.files.length > 0) {
                this.data.tradeMarkImageUrl = this.serverHost + res.files[0].ftpPath
                this.businessLicenseClear = true
            } else if (res.files.length == 0) {
                this.businessLicenseClear = false
            }
        ...
        tradeUseChange(res) {
            if (res.files.length > 0) {
                this.data.tradeUseImageUrl = this.serverHost + res.files[0].ftpPath
                this.businessLicenseClear = true
            } else if (res.files.length == 0) {
                this.businessLicenseClear = false
            }
        ...
        brandAuthorizationChange(res) {
            if (res.files.length > 0) {
                this.data.brandAuthorizationImageUrl = this.serverHost + res.files[0].ftpPath
                this.businessLicenseClear = true
            } else if (res.files.length == 0) {
                this.businessLicenseClear = false
            }
