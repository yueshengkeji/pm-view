          ...
          <v-menu ref="menu3" v-model="menu3" offset-y :close-on-content-click="false">
            ...
            <v-list dense>
              ...
              <v-list-group :value="true">
                ...
                <v-list-item v-for="(type) in typeItems" :key="type.id" v-model="type.active" @click="filterTypeHandler(type)">
    ...
    <v-dialog v-model="bzjDialog" @click="bzjDialog = false" width="30%">
      <v-card class="pa-5">
        ...
        <v-form ref="bzjForm">
          ...
          <v-autocomplete label="类型" :rules="bzjRules.type" :items="bzjTypeItems" :search-input.sync="searchBzjType"
            auto-select-first v-model="bzj.type"></v-autocomplete>
          <v-menu v-model="bzjMenu" :close-on-content-click="false" transition="scale-transition" offset-y
            min-width="auto">
            <template v-slot:activator="{ on, attrs }">
              <v-text-field v-model="bzj.datetime" :rules="bzjRules.datetime" label="日期" readonly v-bind="attrs"
                v-on="on"></v-text-field>
            </template>
            <v-date-picker @change="bzjMenu = false" v-model="bzj.datetime" no-title scrollable></v-date-picker>
          </v-menu>
